...
#'
#' @param ... named data.frames, each with \code{bay_segment}, \code{yr}, and
...
#' )
anlz_indicators <- function(..., bay_segments = c('OTB', 'HB', 'MTB', 'LTB')) {
  ...
  out <- dots |>
    ...
    dplyr::bind_rows() |>
    dplyr::filter(.data$bay_segment %in% bay_segments, !is.na(.data$outcome))
