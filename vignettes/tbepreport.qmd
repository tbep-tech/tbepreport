...
report card: raw monitoring data becomes a 0-1 outcome for each indicator,
...
`plot_score()` plots the full hierarchy - indicators, then categories,
then the overall score - as a sunburst for one bay segment and year:

```{r}
plot_score(wqoverall, sedoverall, fwoverall, haboverall, bay_segment = 'OTB', yr = 2024)
```

`plot_trend()` plots the same hierarchy across every year instead, for
one bay segment:
